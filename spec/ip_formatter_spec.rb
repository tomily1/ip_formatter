...
describe IpFormatter do
  context 'when valid text file is uploaded result:' do
    before do
    ...
    it 'should merge sub addresses of ip with the same ip address' do
      formatter = IpFormatter.new('sample_file2')
      expect(formatter.prepare_file).to eq ["1.2.3.5:25,7,8", "1.2.3.5:25,7,8,8,1", "1.2.3.6:10,1,12,1,1,14,2,1,9,5,7"]
      expect(formatter.display_result).to eq ["1.2.3.5:1,7,8,25", "1.2.3.6:1,1,1,1,2,5,7,9,10,12,14"]
    end

    it 'should return full ip address with sorted subaddresses' do
      formatter = IpFormatter.new('sample_file2')
      expect(formatter.display_result.first).to eq '1.2.3.5:1,7,8,25'
      expect(formatter.display_result.last).to eq '1.2.3.6:1,2,5,7,9,10,12,14'
    end
